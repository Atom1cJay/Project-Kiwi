...
public class MovementParticleExecuter : MonoBehaviour
{
    ...
    private void Update()
    {
        MovementParticleInfo.MovementParticles[] particles = MoveExecuter.instance.GetCurrentMove().GetParticlesToSpawn();
        if (particles != null)
        {
        ...
        MovementParticleInfo.MovementParticles[] toStop = MoveExecuter.instance.GetCurrentMove().GetParticlesToStop();
        if (toStop != null)
        {
